"""Automated Sequential Pipeline for VGG-16-BN CIFAR-100 3-Way Triangulation Benchmark.
...
TSRX_CKPT = Path("results/tsrx/vgg16bn_cifar100_two_regime.pt")
STATIC_MATCHED_CKPT = Path("results/static_matched/vgg16bn_cifar100_two_regime.pt")
REFERENCE_CKPT = Path("results/reference/vgg16bn_cifar100.pt")
...
def run_arm2(epochs: int = 100, batch_size: int = 128, num_workers: int = 0):
    ...
    print("  Architecture: VGG-16-BN | Dataset: CIFAR-100", flush=True)
    print(f"{'='*70}\n", flush=True)
    cmd = [
...
def run_arm3(epochs: int = 100, batch_size: int = 128, num_workers: int = 0):
    ...
    print("  Architecture: VGG-16-BN (Discovered Topology) | Dataset: CIFAR-100", flush=True)
    print(f"{'='*70}\n", flush=True)
    cmd = [
...
def main():
    ...
    print("\nEvaluating VGG-16 CIFAR-100 3-Way Triangulation Summary...")
    eval_script = Path(__file__).parent / "eval_vgg16_cifar100_triangulation.py"
    if eval_script.exists():
        subprocess.run([sys.executable, str(eval_script)])

    # Generate PDF report
    report_script = Path(__file__).parent / "generate_vgg16_cifar100_pdf_report.py"
    if report_script.exists():
        print("\nGenerating VGG-16 CIFAR-100 PDF and Markdown Benchmark Reports...")
