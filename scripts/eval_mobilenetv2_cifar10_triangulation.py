"""Evaluation and Triangulation Summary for MobileNetV2 on CIFAR-10 Benchmark."""
...
def main():
    ...
    print("Evaluating Arm 1: Standard Static Reference Baseline (MobileNetV2)...")
    ck1 = torch.load("results/reference/mobilenetv2_cifar10.pt", map_location="cpu", weights_only=False)
    m1 = build_model("mobilenet_v2", 10, cifar_stem=True).to(device)
    ...
    print("Evaluating Arm 3: C2 Static Matched Control (MobileNetV2)...")
    ck3 = torch.load("results/static_matched/mobilenetv2_cifar10_two_regime.pt", map_location="cpu", weights_only=False)
    ck2_raw = torch.load("results/tsrx/mobilenetv2_cifar10_two_regime.pt", map_location="cpu", weights_only=False)
    widths = ck2_raw["discovered_widths"]
    m3 = build_model("mobilenet_v2", 10, cifar_stem=True)
    ...
    print("Evaluating Arm 2: TSR-X Dynamic Plasticity (MobileNetV2)...")
    ck2 = ck2_raw
    m2 = build_model("mobilenet_v2", 10, cifar_stem=True)
    ...
    print("\n" + "="*85)
    print("      MOBILENETV2 CIFAR-10 3-WAY TRIANGULATION BENCHMARK SUMMARY")
    print("="*85)
    ...
    out_path = Path("results/mobilenetv2_cifar10_triangulation_summary.pt")
