"""Evaluation and Triangulation Summary for VGG-16-BN on Tiny-ImageNet-200 Benchmark."""
...
@torch.no_grad()
def eval_accuracies(model, loader, device="cuda", batch_tf=None):
    model.eval()
...
def main():
    ...
    batch_tf = GPUBatchTransform(is_train=False).to(device)

    # 1. Arm 1: Standard Static Reference
    print("Evaluating Arm 1: Standard Static Reference Baseline (VGG-16-BN Tiny-ImageNet)...")
    ck1 = torch.load("results/reference/vgg16bn_tiny_imagenet.pt", map_location="cpu", weights_only=False)
    m1 = build_model("vgg16_bn", 200).to(device)
    ...
    top1_1, top5_1 = eval_accuracies(m1, val_loader, device=device, batch_tf=batch_tf)
    del m1

    # 2. Arm 3: C2 Static Matched Control
    print("Evaluating Arm 3: C2 Static Matched Control (VGG-16-BN Tiny-ImageNet)...")
    ck3 = torch.load("results/static_matched/vgg16bn_tiny_imagenet_two_regime.pt", map_location="cpu", weights_only=False)
    ck2_raw = torch.load("results/tsrx/vgg16bn_tiny_imagenet_two_regime.pt", map_location="cpu", weights_only=False)
    widths = ck2_raw["discovered_widths"]
    m3 = build_model("vgg16_bn", 200)
    ...
    top1_3, top5_3 = eval_accuracies(m3, val_loader, device=device, batch_tf=batch_tf)
    del m3

    # 3. Arm 2: TSR-X Dynamic Plasticity
    print("Evaluating Arm 2: TSR-X Dynamic Plasticity (VGG-16-BN Tiny-ImageNet)...")
    ck2 = ck2_raw
    m2 = build_model("vgg16_bn", 200)
    ...
    top1_2, top5_2 = eval_accuracies(m2, val_loader, device=device, batch_tf=batch_tf)
    del m2

    print("\n" + "="*95)
    print("  VGG-16-BN TINY-IMAGENET-200 RIGOROUS 3-WAY TRIANGULATION BENCHMARK RESULTS")
    print("="*95)
    ...
    torch.save(summary_data, "results/vgg16bn_tiny_imagenet_triangulation_summary.pt")
    print("Summary saved to results/vgg16bn_tiny_imagenet_triangulation_summary.pt")
