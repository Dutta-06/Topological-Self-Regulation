"""Evaluation and Triangulation Summary for EfficientNet-B0 on CIFAR-10 Benchmark."""
...
def main():
    ...
    _, val_loader = get_cifar10_loaders(batch_size=128)

    # 1. Arm 1: Standard Static Reference
    print("Evaluating Arm 1: Standard Static Reference Baseline (EfficientNet-B0)...")
    ck1 = torch.load("results/reference/efficientnet_b0_cifar10.pt", map_location="cpu", weights_only=False)
    m1 = build_model("efficientnet_b0", 10, cifar_stem=True).to(device)
    ...
    print("Evaluating Arm 3: C2 Static Matched Control (EfficientNet-B0)...")
    ck3 = torch.load("results/static_matched/efficientnet_b0_cifar10_two_regime.pt", map_location="cpu", weights_only=False)
    ck2_raw = torch.load("results/tsrx/efficientnet_b0_cifar10_two_regime.pt", map_location="cpu", weights_only=False)
    widths = ck2_raw["discovered_widths"]
    m3 = build_model("efficientnet_b0", 10, cifar_stem=True)
    ...
    print("Evaluating Arm 2: TSR-X Dynamic Plasticity (EfficientNet-B0)...")
    ck2 = ck2_raw
    m2 = build_model("efficientnet_b0", 10, cifar_stem=True)
    ...
    print("\n" + "="*85)
    print("      EFFICIENTNET-B0 CIFAR-10 3-WAY TRIANGULATION BENCHMARK SUMMARY")
    print("="*85)
    ...
    out_path = Path("results/efficientnet_b0_cifar10_triangulation_summary.pt")
