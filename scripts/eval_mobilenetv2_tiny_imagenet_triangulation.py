"""Evaluation and Triangulation Summary for MobileNetV2 on Tiny-ImageNet-200 Benchmark."""
...
def main():
    ...
    print("Evaluating Arm 1: Standard Static Reference Baseline (MobileNetV2 Tiny-ImageNet)...")
    ck1 = torch.load("results/reference/mobilenetv2_tiny_imagenet.pt", map_location="cpu", weights_only=False)
    m1 = build_model("mobilenet_v2", 200, cifar_stem=False).to(device)
    ...
    print("Evaluating Arm 3: C2 Static Matched Control (MobileNetV2 Tiny-ImageNet)...")
    ck3 = torch.load("results/static_matched/mobilenetv2_tiny_imagenet_two_regime.pt", map_location="cpu", weights_only=False)
    ck2_raw = torch.load("results/tsrx/mobilenetv2_tiny_imagenet_two_regime.pt", map_location="cpu", weights_only=False)
    widths = ck2_raw["discovered_widths"]
    m3 = build_model("mobilenet_v2", 200, cifar_stem=False)
    ...
    print("Evaluating Arm 2: TSR-X Dynamic Plasticity (MobileNetV2 Tiny-ImageNet)...")
    ck2 = ck2_raw
    m2 = build_model("mobilenet_v2", 200, cifar_stem=False)
    ...
    print("\n" + "="*85)
    print("      MOBILENETV2 TINY-IMAGENET-200 3-WAY TRIANGULATION BENCHMARK SUMMARY")
    print("="*85)
    ...
    out_path = Path("results/mobilenetv2_tiny_imagenet_triangulation_summary.pt")
