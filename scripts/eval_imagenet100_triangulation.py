...
def main():
    ...
    _, val_loader = get_imagenet100_loaders(batch_size=64)

    # 1. Arm 1: Standard Static Reference
    print("Evaluating Arm 1: Standard Static Reference Baseline...")
    ck1 = torch.load("results/reference/resnet18_imagenet100.pt", map_location="cpu", weights_only=False)
    m1 = build_model("resnet18", 100, cifar_stem=False).to(device)
    ...
    ck3 = torch.load("results/static_matched/resnet18_imagenet100_two_regime.pt", map_location="cpu", weights_only=False)
    widths = torch.load("results/tsrx/resnet18_imagenet100_two_regime.pt", map_location="cpu", weights_only=False)["discovered_widths"]
    m3 = build_model("resnet18", 100, cifar_stem=False)
    ...
    ck2 = torch.load("results/tsrx/resnet18_imagenet100_two_regime.pt", map_location="cpu", weights_only=False)
    m2 = build_model("resnet18", 100, cifar_stem=False)
    ...
    print("\n" + "="*95)
    print("  IMAGENET-100 RIGOROUS 3-WAY TRIANGULATION BENCHMARK RESULTS")
    print("="*95)
    ...
    }, "results/imagenet100_triangulation_summary.pt")
