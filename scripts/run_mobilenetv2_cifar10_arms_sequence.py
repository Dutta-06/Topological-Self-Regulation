"""Automated Sequential Pipeline for MobileNetV2 CIFAR-10 3-Way Triangulation Benchmark.

Sequentially executes:
  1. Arm 2 (TSR-X Dynamic Plasticity) with budget cap B(t) = 0.85 * baseline (~2.2M params).
...
TSRX_CKPT = Path("results/tsrx/mobilenetv2_cifar10_two_regime.pt")
STATIC_MATCHED_CKPT = Path("results/static_matched/mobilenetv2_cifar10_two_regime.pt")
REFERENCE_CKPT = Path("results/reference/mobilenetv2_cifar10.pt")
...
def run_arm2(epochs: int = 100, batch_size: int = 128, num_workers: int = 0):
    ...
    print("  Architecture: MobileNetV2 | Dataset: CIFAR-10", flush=True)
    print(f"{'='*70}\n", flush=True)
    cmd = [
...
def run_arm3(epochs: int = 100, batch_size: int = 128, num_workers: int = 0):
    ...
    print("  Architecture: MobileNetV2 (Discovered Topology) | Dataset: CIFAR-10", flush=True)
    print(f"{'='*70}\n", flush=True)
    cmd = [
...
def main():
    ...
    print("\nEvaluating MobileNetV2 CIFAR-10 3-Way Triangulation Summary...")
    eval_script = Path(__file__).parent / "eval_mobilenetv2_cifar10_triangulation.py"
    if eval_script.exists():
        subprocess.run([sys.executable, str(eval_script)])

    # Generate PDF report
    report_script = Path(__file__).parent / "generate_mobilenetv2_cifar10_pdf_report.py"
    if report_script.exists():
        print("\nGenerating MobileNetV2 CIFAR-10 PDF and Markdown Benchmark Reports...")
