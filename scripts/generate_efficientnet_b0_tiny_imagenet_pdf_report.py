"""Generate High-Resolution Publication-Quality PDF and Markdown Reports for EfficientNet-B0 on Tiny-ImageNet-200."""
...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_number(self, page_count):
        ...
        self.drawString(54, 32, "Empirical Research • EfficientNet-B0 Tiny-ImageNet-200 Benchmark")
        self.restoreState()


def generate_plots(output_dir: Path, data: dict):
    output_dir.mkdir(parents=True, exist_ok=True)
    plot_path = output_dir / "efficientnet_b0_tiny_imagenet_pareto.png"
    ...
    ax.set_title("EfficientNet-B0 Tiny-ImageNet-200: Accuracy vs Active Parameters", fontsize=12, fontweight="bold", pad=12)
...
def build_pdf(pdf_path: Path, data: dict, plot_path: Path):
    ...
    story = []
    story.append(Paragraph("TSR-X Benchmark: EfficientNet-B0 on Tiny-ImageNet-200", title_style))
...
def build_markdown(md_path: Path, data: dict):
    ...
    content = f"""# TSR-X Benchmark Report: EfficientNet-B0 on Tiny-ImageNet-200
...
def main():
    summary_path = Path("results/efficientnet_b0_tiny_imagenet_triangulation_summary.pt")
    ...
    pdf_path = out_dir / "EFFICIENTNET_B0_TINY_IMAGENET_BENCHMARK_REPORT.pdf"
    md_path = out_dir / "EFFICIENTNET_B0_TINY_IMAGENET_BENCHMARK_REPORT.md"
