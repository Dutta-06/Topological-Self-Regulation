"""Automated Sequential Pipeline for ImageNet-100 3-Way Triangulation Benchmark.

Sequentially executes:
  1. Arm 2 (TSR-X Dynamic Plasticity) with budget cap B(t) = 0.85 * baseline.
  ...
  4. Runs evaluation and generates markdown and PDF reports.
"""
...
TSRX_CKPT = Path("results/tsrx/resnet18_imagenet100_two_regime.pt")
STATIC_MATCHED_CKPT = Path("results/static_matched/resnet18_imagenet100_two_regime.pt")
REFERENCE_CKPT = Path("results/reference/resnet18_imagenet100.pt")


def run_arm2(epochs: int = 100, batch_size: int = 64, num_workers: int = 4):
    print(f"\n{'='*70}", flush=True)
...
def run_arm3(epochs: int = 100, batch_size: int = 64, num_workers: int = 4):
    print(f"\n{'='*70}", flush=True)
...
def run_arm1(epochs: int = 100, batch_size: int = 64, num_workers: int = 4):
    print(f"\n{'='*70}", flush=True)
...
def main():
    ...
    print("\nEvaluating 3-Way Triangulation Summary...")
    eval_script = Path(__file__).parent / "eval_imagenet100_triangulation.py"
    subprocess.run([sys.executable, str(eval_script)])

    # Generate PDF report
    report_script = Path(__file__).parent / "generate_imagenet100_pdf_report.py"
    if report_script.exists():
        print("\nGenerating ImageNet-100 PDF and Markdown Benchmark Reports...")
