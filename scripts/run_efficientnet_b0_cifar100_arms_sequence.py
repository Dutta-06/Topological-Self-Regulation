"""Automated Sequential Pipeline for EfficientNet-B0 CIFAR-100 3-Way Triangulation Benchmark."""
...
TSRX_CKPT = Path("results/tsrx/efficientnet_b0_cifar100_two_regime.pt")
STATIC_MATCHED_CKPT = Path("results/static_matched/efficientnet_b0_cifar100_two_regime.pt")
REFERENCE_CKPT = Path("results/reference/efficientnet_b0_cifar100.pt")
...
def run_arm2(epochs: int = 100, batch_size: int = 128, num_workers: int = 0):
    ...
    print("  Architecture: EfficientNet-B0 | Dataset: CIFAR-100", flush=True)
    print(f"{'='*70}\n", flush=True)
    cmd = [
...
def run_arm3(epochs: int = 100, batch_size: int = 128, num_workers: int = 0):
    ...
    print("  Architecture: EfficientNet-B0 (Discovered Topology) | Dataset: CIFAR-100", flush=True)
    print(f"{'='*70}\n", flush=True)
    cmd = [
...
def main():
    ...
    eval_script = Path(__file__).parent / "eval_efficientnet_b0_cifar100_triangulation.py"
    if eval_script.exists():
        subprocess.run([sys.executable, str(eval_script)])

    report_script = Path(__file__).parent / "generate_efficientnet_b0_cifar100_pdf_report.py"
