"""Automated Sequential Pipeline for MobileNetV2 Tiny-ImageNet-200 3-Way Triangulation Benchmark."""
...
TSRX_CKPT = Path("results/tsrx/mobilenetv2_tiny_imagenet_two_regime.pt")
STATIC_MATCHED_CKPT = Path("results/static_matched/mobilenetv2_tiny_imagenet_two_regime.pt")
REFERENCE_CKPT = Path("results/reference/mobilenetv2_tiny_imagenet.pt")


def run_arm2(epochs: int = 100, batch_size: int = 64, num_workers: int = 0):
    print(f"\n{'='*70}", flush=True)
    print("  LAUNCHING ARM 2: TSR-X DYNAMIC PLASTICITY (Online Channel Discovery)", flush=True)
    print("  Architecture: MobileNetV2 | Dataset: Tiny-ImageNet-200", flush=True)
    print(f"{'='*70}\n", flush=True)
    cmd = [
...
def run_arm3(epochs: int = 100, batch_size: int = 64, num_workers: int = 0):
    print(f"\n{'='*70}", flush=True)
    print("  LAUNCHING ARM 3: C2 STATIC MATCHED CONTROL (Trained from Scratch)", flush=True)
    print("  Architecture: MobileNetV2 (Discovered Topology) | Dataset: Tiny-ImageNet-200", flush=True)
    print(f"{'='*70}\n", flush=True)
    cmd = [
...
def run_arm1(epochs: int = 100, batch_size: int = 64, num_workers: int = 0):
    print(f"\n{'='*70}", flush=True)
...
def main():
    ...
    print("\nEvaluating MobileNetV2 Tiny-ImageNet-200 3-Way Triangulation Summary...")
    eval_script = Path(__file__).parent / "eval_mobilenetv2_tiny_imagenet_triangulation.py"
    if eval_script.exists():
        subprocess.run([sys.executable, str(eval_script)])

    report_script = Path(__file__).parent / "generate_mobilenetv2_tiny_imagenet_pdf_report.py"
    if report_script.exists():
        print("\nGenerating MobileNetV2 Tiny-ImageNet-200 PDF and Markdown Benchmark Reports...")
