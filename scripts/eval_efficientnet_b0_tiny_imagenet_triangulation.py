"""Evaluation and Triangulation Summary for EfficientNet-B0 on Tiny-ImageNet-200 Benchmark."""
...
def count_flops(model, input_size=(1, 3, 64, 64), device="cuda"):
    flops = 0
    hooks = []
...
@torch.no_grad()
def eval_accuracies(model, loader, eval_tf, device="cuda"):
    model.eval()
    top1, top5, total = 0, 0, 0
    for x, y in loader:
        x, y = eval_tf(x.to(device, non_blocking=True)), y.to(device, non_blocking=True)
...
def main():
    ...
    _, val_loader = get_tiny_imagenet_loaders(batch_size=64)
    eval_tf = GPUBatchTransform(is_train=False).to(device)

    # 1. Arm 1: Standard Static Reference
    print("Evaluating Arm 1: Standard Static Reference Baseline (EfficientNet-B0 Tiny-ImageNet)...")
    ck1 = torch.load("results/reference/efficientnet_b0_tiny_imagenet.pt", map_location="cpu", weights_only=False)
    m1 = build_model("efficientnet_b0", 200, cifar_stem=False).to(device)
    m1.load_state_dict(ck1["model_state_dict"])
    p1 = sum(p.numel() for p in m1.parameters() if p.requires_grad)
    flops1 = count_flops(m1, input_size=(1, 3, 64, 64), device=device)
    top1_1, top5_1 = eval_accuracies(m1, val_loader, eval_tf, device=device)
    del m1

    # 2. Arm 3: C2 Static Matched Control
    print("Evaluating Arm 3: C2 Static Matched Control (EfficientNet-B0 Tiny-ImageNet)...")
    ck3 = torch.load("results/static_matched/efficientnet_b0_tiny_imagenet_two_regime.pt", map_location="cpu", weights_only=False)
    ck2_raw = torch.load("results/tsrx/efficientnet_b0_tiny_imagenet_two_regime.pt", map_location="cpu", weights_only=False)
    widths = ck2_raw["discovered_widths"]
    m3 = build_model("efficientnet_b0", 200, cifar_stem=False)
    m3 = resize_model_to_widths(m3, widths, torch.zeros(2, 3, 64, 64)).to(device)
    m3.load_state_dict(ck3["model_state_dict"])
    p3 = sum(p.numel() for p in m3.parameters() if p.requires_grad)
    flops3 = count_flops(m3, input_size=(1, 3, 64, 64), device=device)
    top1_3, top5_3 = eval_accuracies(m3, val_loader, eval_tf, device=device)
    del m3

    # 3. Arm 2: TSR-X Dynamic Plasticity
    print("Evaluating Arm 2: TSR-X Dynamic Plasticity (EfficientNet-B0 Tiny-ImageNet)...")
    ck2 = ck2_raw
    m2 = build_model("efficientnet_b0", 200, cifar_stem=False)
    m2 = resize_model_to_widths(m2, widths, torch.zeros(2, 3, 64, 64)).to(device)
    sd2 = {}
    ...
    flops2 = count_flops(m2, input_size=(1, 3, 64, 64), device=device)
    top1_2, top5_2 = eval_accuracies(m2, val_loader, eval_tf, device=device)
    del m2
    ...
    print("\n" + "="*85)
    print("      EFFICIENTNET-B0 TINY-IMAGENET-200 3-WAY TRIANGULATION BENCHMARK SUMMARY")
    print("="*85)
    ...
    out_path = Path("results/efficientnet_b0_tiny_imagenet_triangulation_summary.pt")
    torch.save(summary_data, out_path)
    print(f"Summary tensor saved to: {out_path}")
