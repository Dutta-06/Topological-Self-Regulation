...
def evaluate_exchange(
    ...
    # Case A: Budget Slack — Pure Growth allowed if cost fits within budget
    if best_g and budget_params is not None:
        cost_after_grow = current_params + best_g["kappa"]
        if cost_after_grow <= budget_params and best_g["kappa"] * best_g["gamma"] > delta:
    ...
    # Case B: Pure Removal allowed if unit has near-zero saliency (decayed / dead)
    if worst_p and worst_p["kappa"] * worst_p["rho"] < prune_tolerance:
        return ExchangeDecision(
            action="pure_prune",
            prune_tap=worst_p["tap"],
            prune_idx=worst_p["unit_idx"],
            prune_rho=worst_p["rho"],
            delta_loss_bound=worst_p["kappa"] * worst_p["rho"],
            details=worst_p,
        )
